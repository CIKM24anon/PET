...
class ItemTrainDataset(Dataset):
    def __init__(self, conf, u_i_pairs, u_i_graph, num_items, u_i_for_neg_sample, i_i_for_neg_sample, neg_sample = 1):
        ...
        self.u_i_for_neg_sample = u_i_for_neg_sample
...
class BundleTrainDataset(Dataset):
    def __init__(self, conf, u_b_pairs, u_i_pairs, u_b_graph, u_i_graph, num_bundles, num_items, u_b_for_neg_sample, u_i_for_neg_sample, b_b_for_neg_sample, i_i_for_neg_sample, name, neg_sample):
        self.conf = conf
        ...
        while True:
            ...
            if self.u_b_graph[user_b, i] == 0 and not i in all_bundles:                                                          
                ...
                if len(all_bundles) == self.neg_sample+1:                                                                               
                    break
        try:
            all_items = [random.choice(list(self.ui[user_b]))]
        except:
            all_items = []

        while True:
            j = np.random.randint(self.num_items)
            if self.u_i_graph[user_b, j] == 0 and not j in all_items:
                all_items.append(j)
                if len(all_items) == self.neg_sample+1:
                    break
...
class Datasets():
    def __init__(self, conf):
        ...
        u_i_for_neg_sample, i_i_for_neg_sample = None, None
        
        self.bundle_train_data = BundleTrainDataset(conf, u_b_pairs_train, u_i_pairs, u_b_graph_train, u_i_graph, self.num_bundles, self.num_items, u_b_for_neg_sample, u_i_for_neg_sample, b_b_for_neg_sample, i_i_for_neg_sample, conf['dataset'], conf['neg_num'])
        ...
        self.item_train_data = ItemTrainDataset(conf, u_i_pairs, u_i_graph, self.num_items, u_i_for_neg_sample, i_i_for_neg_sample, conf['neg_num'])
